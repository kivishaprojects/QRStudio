...
// Sends a contact-form enquiry to the business inbox (best-effort).
export async function sendEnquiry({ name, email, phone, subject, message }) {
  const to = process.env.CONTACT_TO || "kivishaprojects@gmail.com";
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#16224c">
      <h2 style="margin:0 0 12px">New enquiry — India QR Code</h2>
      <table style="width:100%;border-collapse:collapse;font-size:14px">
        <tr><td style="padding:6px 0;color:#5b6685">Name</td><td><b>${name || "—"}</b></td></tr>
        <tr><td style="padding:6px 0;color:#5b6685">Email</td><td>${email || "—"}</td></tr>
        <tr><td style="padding:6px 0;color:#5b6685">Phone</td><td>${phone || "—"}</td></tr>
        <tr><td style="padding:6px 0;color:#5b6685">Subject</td><td>${subject || "—"}</td></tr>
      </table>
      <p style="margin:14px 0 4px;color:#5b6685">Message</p>
      <div style="white-space:pre-wrap;background:#f1f5fb;border:1px solid #e2e8f3;border-radius:10px;padding:12px;font-size:14px">${(message || "").replace(/</g, "&lt;")}</div>
    </div>`;
  await sendEmail({ to, subject: "Enquiry: " + (subject || "Website contact form"), html });
}
