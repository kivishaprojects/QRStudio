...
async function sendEmail({ to, subject, html }) {
  ...
  if (!key || !to) return;
  const from = process.env.RESEND_FROM || "QR Studio <onboarding@resend.dev>";
  try {
...
export async function sendOrderReceipt(admin, orderId) {
  try {
    ...
    let RATE = 18, BIZ = "QR Studio";
    try {
      const { data: st } = await admin.from("qs_settings").select("gst_rate,biz_name").eq("id", 1).single();
      if (st) { if (st.gst_rate != null) RATE = Number(st.gst_rate); if (st.biz_name) BIZ = st.biz_name; }
    } catch (_) {}
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;color:#1b2138">
        <h2 style="margin:0 0 4px">QR Studio — Payment Receipt</h2>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          ...
          <tr><td style="padding:6px 0;color:#5f6982">GST @ ${RATE}% (CGST+SGST)</td><td style="text-align:right">₹${gst}</td></tr>
    ...
    await sendEmail({ to, subject: "Your QR Studio receipt — " + (o.invoice_no || o.id), html });
  } catch (_) {}
}
