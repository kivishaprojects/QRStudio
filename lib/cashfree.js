...
export async function verifyGstin(gstin) {
  ...
  if (!res.ok) {
    // Surface Cashfree's real code so the failure is diagnosable (logged server-side).
    const code = d.code || d.type || "";
    console.error("Cashfree GSTIN verify failed", { status: res.status, code, message: d.message });
    const err = new Error(d.message || "GSTIN verification failed");
